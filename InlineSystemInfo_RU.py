...
@loader.tds
class InlineSystemInfoMod_RU(loader.Module):
    """🖥 Получите подробную информацию о вашем сервере | Перевод не полный и в некоторых местах не правильный."""
    ...
    def general_info(self):
        string = "😼 <b>Системная информация</b>\n"
        string += f"  ├──<b>Имя процессора</b>: <code>{self.cpu_info.get('brand_raw', 'Undetermined.')}</code> {self.cpu_count_logic}/{self.cpu_count} ({self.cpu_info['arch_string_raw']})\n"
        string += f"  ├──<b>ОЗУ</b>: {progressbar(self.virtual_memory.percent, 10)} <code>({bytes2human(self.virtual_memory.used)}/{bytes2human(self.virtual_memory.total)})</code>\n"
        ...
        string += "🐧 <b>Информация о Linux</b>\n"
        string += f"  ├──<b>Имя</b>: <code>{get_distro()}</code>\n"
        ...
        if hasattr(self, "disk_partitions"):
            for disk_root in psutil.disk_partitions("/"):
                if disk_root.mountpoint == '/':
                    ...
                    string += "💽 <b>Информация о диске</b> <code>(/)</code>\n"
                    string += f"  └──<b>{disk_root.device}</b>\n"
                    string += f"        ├── <b>Mount</b> {disk_root.mountpoint}\n"
                    string += f"        ├── <b>FS</b> {disk_root.fstype}\n"
                    string += f"        ├── <b>Использование диска</b> {disk_usage.percent}% ({bytes2human(disk_usage.used)}/{bytes2human(disk_usage.total)})\n"
                    string += f"        │       └──{progressbar(disk_usage.percent, 10)}\n"
                    string += f"        └── <b>Параметры</b> {disk_root.opts}\n\n"

        return string

    def cpu_string(self):
        string = "🧠  <b>Информация о процессоре</b>\n"
        string += f"⦁ <b>Имя</b>: {self.cpu_info.get('brand_raw', 'Undetermined.')} ({self.cpu_info['arch_string_raw']})\n"
        string += f"⦁ <b>Ядра</b>: {self.cpu_count_logic} ({self.cpu_count})\n"
        string += (
    ...
    def disks_string(self):
        ...
        for disk in self.disk_partitions:
            ...
            string += f"├── <b>Память</b> {disk_usage.percent}% ({bytes2human(disk_usage.used)}/{bytes2human(disk_usage.total)})\n"
            string += f"│       └──{progressbar(disk_usage.percent, 10)}\n"
    ...
    def memory_string(self):
        string = "🗄  <b>Информация о памяти</b>\n"
    ...
    def sensors_string(self):
        ...
        if hasattr(self, "sensors_temperatures"):
            string = "🌡  <b>Информация о датчиках</b>\n" + "<b>Температура</b>:\n"
    ...
    def linux_string(self):
        ...
        string = f"""🐧  <b>Информация о Linux</b>
        Name: {get_distro()}
    ...
    def python_string(self):
        string = "🐍 <b>Информация о Python</b>\n"
